...
- **Popup Dashboard** — same layout grammar as the MPD and Home Assistant panels: a compact header, text tabs, and thin-bordered cards on the panel fill.
  - **Overview**: a single **bandwidth** area chart of total rclone throughput (all processes, mounts included; sampled from `/proc/<pid>/io`); a compact **mounts** summary that jumps to the Mounts tab; and the next-timer card with a **Sync now** button. A running transfer is signalled by the LED alone — no separate card.
  - Cards for scheduled jobs and mounts carry a **corner tag** (top-right) naming the kind: one-way sync / bi-directional / copy / move / check, or `rw` / `ro` for a mount.
  ...
  - **Storages**: configured remote cards with capacity bars (used / free / total / trash).
